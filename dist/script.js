...
async function getCountries() {
    const data = await getData(`${URL_BASE}/countries`);
    state.countries = data.map(normalizeCountriesAPI);
}
async function init() {
    await getCountries();
    const suggestions = new Suggestions(state.countries, ".header-section", ".search-container");
    ...
    function getCountryClicked(e) {
        if (!(e.target instanceof HTMLLIElement && inputSearch))
            return;
